// Header.jsx — com Link "Ver Carrinho" levando à página completa
...
function SecondaryNavbar({ isMobile = false }) {
  return (
    <ul className={`flex flex-col lg:flex-row ${isMobile ? 'space-y-4' : 'space-x-6'} text-sm font-medium`}>
      <li><Link to="/" className="text-pink-600 border-b-2 border-pink-600 pb-1">Home</Link></li>
      <li><Link to="/produtos" className="text-gray-700 hover:text-pink-500">Produtos</Link></li>
      <li><Link to="/categorias" className="text-gray-700 hover:text-pink-500">Categorias</Link></li>
      <li><Link to="/meus-pedidos" className="text-gray-700 hover:text-pink-500">Meus Pedidos</Link></li>
    </ul>
  );
}
...
export default function Header() {
  ...
  /* Dropdown do carrinho (mobile + desktop) */
  const CartDropdown = () => (
    <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border z-50">
        ...
        {cart.length === 0 ? (
          ...
          cart.map((item) => (
            <div key={item.id} className="flex items-center gap-3 border-b py-3">
              <img src={item.image || 'https://via.placeholder.com/100'} alt={item.name} className="w-14 h-14 object-contain border rounded" />
      ...
      <div className="px-6 py-4 border-t">
        ...
        <div className="flex justify-between items-center">
          <button onClick={clearCart} disabled={cart.length === 0} className={`text-xs underline ${cart.length === 0 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:text-gray-800'}`}>Esvaziar</button>
          <Link to="/carrinho">
    ...
    <header className="bg-white shadow-sm w-full relative z-50">
      <div className="max-w-[993px] mx-auto px-6 py-6 flex flex-col">
        ...
        <div className="flex items-center justify-between w-full gap-6">
          <button className="lg:hidden text-gray-800" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}</button>
          <div className="flex-grow flex justify-center lg:justify-start items-center"><img src={logo} alt="Logo Digital Store" className="h-6 md:h-7 lg:h-8" /></div>
          ...
          <div className="hidden lg:flex flex-grow">
            <div className="relative w-full max-w-xl">
              ...
              <button className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"><Search className="w-4 h-4" /></button>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            <Link to="/login"><button className="bg-[#e91e63] hover:bg-pink-600 text-white font-semibold px-5 py-2 rounded-md text-sm" style={{ width: '114px', height: '40px' }}>Entrar</button></Link>
            <div className="relative">
              <button onClick={() => setShowCart(!showCart)}>
                <ShoppingCart className="w-6 h-6 text-pink-600" />
                {cart.length > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">{cart.length}</span>}
